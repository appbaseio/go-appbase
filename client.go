...
// PerformRequest does a HTTP request to Elasticsearch and parses the result as a streaming result.
// It returns a response and an error on failure.
func (c *Client) PerformStreamingRequest(method, path string, params url.Values, body interface{}, responseStream chan *json.RawMessage, errorStream chan error) (*Response, error) {
	start := time.Now().UTC()
	...
	for {
		...
		dec := json.NewDecoder(bodyCopy)

		var initialResponse json.RawMessage
		err = dec.Decode(&initialResponse)
		if err != nil {
			return nil, err
		}
		res.Body = ioutil.NopCloser(bytes.NewReader(initialResponse))
