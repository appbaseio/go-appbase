...
func (s *SearchService) Do() (*SearchResult, chan *json.RawMessage, chan error, error) {
	// Build url
	path := "/"
	...
	res, err := s.client.PerformStreamingRequest("POST", path, params, body, responseChannel, errorChannel)
	if err != nil {
		return nil, nil, nil, err
	}
