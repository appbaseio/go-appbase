...
func TestGet(t *testing.T) {
	...
	// Get document 1
	res, _, _, err := client.Get().Index(testIndexName).Type("tweet").Id("1").Do()
	if err != nil {
		t.Fatal(err)
	}
	if res.Found != true {
	...
	res, _, _, err = client.Get().Index(testIndexName).Type("tweet").Id("99").Do()
	if err != nil {
		t.Fatal(err)
	}
	if res.Found != false {
...
func TestStreamingGet(t *testing.T) {
	client := setupStreamingTestClient(t, SetSniff(false), SetURL("http://testuser:testpass@localhost:7999"))
	...
	// Get document 1
	res, responseStream, _, err := client.Get().Index(testIndexName).Type("tweet").Id("1").Do()
	if err != nil {
		t.Fatal(err)
	}
	if res.Found != true {
...
func TestGetWithSourceFiltering(t *testing.T) {
	...
	res, _, _, err := client.Get().Index(testIndexName).Type("tweet").Id("1").FetchSource(false).Do()
	if err != nil {
		t.Fatal(err)
	}
	if res.Found != true {
	...
	res, _, _, err = client.Get().Index(testIndexName).Type("tweet").Id("1").FetchSourceContext(fsc).Do()
	if err != nil {
		t.Fatal(err)
	}
	if res.Found != true {
...
func TestGetWithFields(t *testing.T) {
	...
	res, _, _, err := client.Get().Index(testIndexName).Type("tweet").Id("1").Fields("message", "_timestamp").Do()
	if err != nil {
		t.Fatal(err)
	}
	if res.Found != true {
...
func TestGetFailsWithMissingParams(t *testing.T) {
	...
	client := setupTestClientAndCreateIndex(t)
	if _, _, _, err := client.Get().Do(); err == nil {
		t.Fatal("expected Get to fail")
	}
	if _, _, _, err := client.Get().Index(testIndexName).Do(); err == nil {
		t.Fatal("expected Get to fail")
	}
	if _, _, _, err := client.Get().Type("tweet").Do(); err == nil {
		t.Fatal("expected Get to fail")
	}
	if _, _, _, err := client.Get().Id("1").Do(); err == nil {
		t.Fatal("expected Get to fail")
	}
	if _, _, _, err := client.Get().Index(testIndexName).Type("tweet").Do(); err == nil {
		t.Fatal("expected Get to fail")
	}
	/*
	...
	*/
	if _, _, _, err := client.Get().Type("tweet").Id("1").Do(); err == nil {
		t.Fatal("expected Get to fail")
	}
}
